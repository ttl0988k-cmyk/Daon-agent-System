...
def build_builder_task(request, build_target=None):
    ...
    task_text = (
        ...
        "  5. 결과의 맨 마지막에 기계 판독 핸드오프 마커를 한 줄로 남긴다. 형식:\n"
        "     [BUILDER_HANDOFF] {\"name\": \"<스킬 이름>\", \"probe_paths\": [\"<프로브 파일 경로>\"], \"status\": \"draft\"}\n"
        "     거버넌스 절차(갭 E-L4)가 이 마커를 파싱해 프로브 검증과 편입의 근거로 사용한다.\n"
    )
    criteria = [
        f"결핍 능력 '{cap}'에 대한 {target} 초안 산출물이 존재한다",
        "초안을 검증하는 프로브/테스트가 작성되어 통과한다",
        "초안은 정식 등록되지 않고 draft 상태로 남는다",
        "결과에 산출물 경로와 사용법, [BUILDER_HANDOFF] 핸드오프 마커가 포함된다",
    ]
