...
if hasattr(sys, '_MEIPASS'):
    ...
    if _appdata:
        ...
        STATE_DIR = HOME / '.daon-agent' / 'data'
else:
...
try:
    ...
    print(f"[WARN] Cannot create STATE_DIR at {STATE_DIR}: {e}", file=sys.stderr)
try:
    SESSION_DIR.mkdir(parents=True, exist_ok=True)
except (PermissionError, OSError) as e:
    print(f"[WARN] Cannot create SESSION_DIR at {SESSION_DIR}: {e}", file=sys.stderr)

# ── Seed data from bundled resources on first run (PyInstaller builds) ──
# When running as a packaged .exe, STATE_DIR is %LOCALAPPDATA%/DAON Agent System/data
# and starts empty.  Copy default files from the bundled data/ directory so the
# user sees the same default providers/models/workspaces they had during development.
if hasattr(sys, '_MEIPASS'):
    _bundled_data = RESOURCE_DIR / 'data'
    if _bundled_data.exists() and _bundled_data.is_dir():
        _seed_files = [
        ...
        for _fname in _seed_files:
            _target = STATE_DIR / _fname
            _source = _bundled_data / _fname
            if not _target.exists() and _source.exists():
                try:
                    import shutil as _shutil
                    _shutil.copy2(str(_source), str(_target))
                    _seeded += 1
                except (PermissionError, OSError) as e:
                    print(f"[WARN] Cannot seed {_fname} to STATE_DIR: {e}", file=sys.stderr)
        if _seeded:
            print(f"[Config] Seeded {_seeded} default data file(s) from bundled resources.", file=sys.stderr)
