...
def _clean_workspace_list(workspaces: list) -> list:
    ...
    for w in workspaces:
        ...
        try:
            ...
            # Expected: path is NOT under profiles/ — keep it
            pass
...
def _migrate_global_workspaces() -> list:
    ...
    try:
        ...
        _logger.warning("Failed to migrate global workspaces from %s", _GLOBAL_WS_FILE, exc_info=True)
        return []
        ...
        try:
            ...
            if len(cleaned) != len(raw):
                ...
                try:
                    ...
                    _logger.warning("Failed to persist cleaned workspaces to %s", ws_file, exc_info=True)
            return cleaned or [{'path': _profile_default_workspace(), 'name': 'Home'}]
        except Exception:
            _logger.warning("Failed to load workspaces from %s", ws_file, exc_info=True)
