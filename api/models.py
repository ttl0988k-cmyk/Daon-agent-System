...
class Session:
    ...
    def to_response(self, include_messages: bool = True, include_tool_calls: bool = True) -> dict:
        """
        통일된 API 응답 형식 (Integrator Contract — Shared Schema).
        모든 세션 라우트는 이 메서드로 응답을 생성해야 한다.

        Returns canonical `session` object matching shared/schema.py:SessionFull.
        """
        result = self.compact()
        if include_messages:
...
def get_cli_sessions() -> list:
    ...
    try:
        ...
        _logger.warning("sqlite3 not available, cannot read CLI sessions")
        return cli_sessions
...
def _save_session_to_db(session: Session) -> None:
    """
    Sync a WebUI session to the SQLite state.db for session_search compatibility.
    
    This ensures WebUI sessions are searchable via the session_search tool.
    Uses WAL mode for better concurrent read/write performance.
    """
    db_path = _get_state_db_path()
    if not db_path.exists():
        return  # DB doesn't exist yet -- will be created by Hermes Agent
    ...
    try:
        ...
        if existing:
            ...
            cursor.execute("""
                INSERT INTO sessions (
                    id, source, model, started_at, updated_at,
        ...
        _logger.warning("Failed to sync session %s to SQLite: %s", session.session_id, e)
