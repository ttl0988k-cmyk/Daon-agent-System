...
def _normalize_for_deepseek(model_name: str) -> str:
    ...
    - Contains any reasoner keyword (r1, think, reasoning, cot, reasoner)
      -> ``deepseek-reasoner``.
    - Matches a known V4/V3/Chat/Jauns/VL pattern -> pass through as-is.
    - Everything else -> ``deepseek-chat`` (safe default for unknown models).

    Args:
        model_name: The bare model name (vendor prefix already stripped).

    Returns:
        A DeepSeek API-compatible model identifier.
    """
    ...
    # Known patterns: deepseek-v4-*, deepseek-v3-*, deepseek-chat, etc.
    if bare.startswith("deepseek-"):
        # Pass through any deepseek-prefixed model that DeepSeek API might support
        return bare
