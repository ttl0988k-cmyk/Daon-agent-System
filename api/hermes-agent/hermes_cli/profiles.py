...
_PROFILE_ID_RE = re.compile(r"^[a-zA-Z0-9가-힣ㄱ-ㅎㅏ-ㅣ()_-]{1,63}$")
...
def create_profile(
    ...
    # Seed a default AGENTS.md so the user has a file to customize immediately.
    # Skipped when the profile already has one (from --clone / --clone-all).
    ...
    if not agents_path.exists():
        try:
            default_agents_content = (
                ...
                "Define your job-specific operating rules, response formats, and protocols here.\n"
                "This file is loaded automatically when this profile is active.\n"
            )
            agents_path.write_text(default_agents_content, encoding="utf-8")
        except Exception:
            pass
