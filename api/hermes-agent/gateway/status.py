...
def _is_process_alive(pid: int) -> bool:
    """Platform-independent check if a process is running."""
    if _IS_WINDOWS:
        try:
            import ctypes
            ...
            if handle == 0:
                # If OpenProcess fails, check if error is ERROR_ACCESS_DENIED (5)
                # If Access Denied, process exists but we can't query it -> assume alive
