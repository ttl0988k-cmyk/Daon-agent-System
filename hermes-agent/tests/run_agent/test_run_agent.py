...
class TestConcurrentToolExecution:
    ...
    def test_single_tool_uses_sequential_path(self, agent):
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tc])
        messages = []
        with patch("agent.tool_executor.execute_tool_calls_sequential") as mock_seq:
            with patch("agent.tool_executor.execute_tool_calls_concurrent") as mock_con:
                agent._execute_tool_calls(mock_msg, messages, "task-1")
...
class TestCredentialPoolRecovery:
    def test_recover_with_pool_rotates_on_402(self, agent):
        ...
        with patch("agent.fallback.swap_credential") as mock_swap:
            ...
            mock_swap.assert_called_once_with(agent, next_entry)
    ...
    def test_recover_with_pool_refreshes_on_401(self, agent):
        ...
        with patch("agent.fallback.swap_credential") as mock_swap:
            ...
            assert recovered is True
            mock_swap.assert_called_once_with(agent, refreshed_entry)
    ...
    def test_recover_with_pool_passes_error_context_on_rotated_429(self, agent):
        ...
        with patch("agent.fallback.swap_credential"):
            recovered, retry_same = agent._recover_with_credential_pool(
                status_code=429,
                has_retried_429=True,
...
class TestStreamingApiCall:
    ...
    def test_api_exception_propagates_no_non_streaming_fallback(self, agent):
        ...
        # patch the module-level function (not the 4-line AIAgent wrapper)
        # because stream_handler.py imports it directly from openai_client_lifecycle
        with patch("agent.openai_client_lifecycle.replace_primary_openai_client", return_value=False):
