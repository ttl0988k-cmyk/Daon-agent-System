...
def _try_openrouter() -> Tuple[Optional[OpenAI], Optional[str]]:
    # ── OpenRouter 차단 가드 ─────────────────────────────────────────
    # OpenRouter 대시보드에 원치 않는 사용량이 청구되는 것을 막기 위해
    # 보조(auto 체인) / resolve_provider_client / 비전 백엔드 등 모든
    # OpenRouter 경로의 단일 진입점인 이 함수를 기본 비활성화한다.
    # 임시로 다시 활성화하려면 환경변수 HERMES_ENABLE_OPENROUTER=1 설정.
    if os.getenv("HERMES_ENABLE_OPENROUTER", "").strip().lower() not in ("1", "true", "yes"):
        logger.debug("Auxiliary client: OpenRouter disabled by guard")
        return None, None
...
def _try_payment_fallback(
    ...
    [DISABLED] Cross-provider fallback disabled by user request to protect credits.

    """

    logger.warning(

        "Auxiliary %s: %s on %s. Cross-provider fallback is DISABLED by user request.",
...
async def async_call_llm(
    ...
    *,
    ...
    # Convert image blocks for Anthropic-compatible endpoints (e.g. xiaomi, zai).
    # DAON FIX: Skip conversion for vision task on OpenAI-compatible endpoints
    # (minimax) — its API uses OpenAI chat.completions format and only accepts
    # image_url blocks, not the Anthropic source.base64 image shape.
    if task != "vision" and _is_anthropic_compat_endpoint(resolved_provider, _client_base):
        kwargs["messages"] = _convert_openai_images_to_anthropic(kwargs["messages"])
