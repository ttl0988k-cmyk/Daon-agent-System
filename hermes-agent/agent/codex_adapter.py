...
#
# Codex Responses API adapter — extracted from AIAgent in run_agent.py.
...
# ── Instance methods (agent-aware) ──────────────────────────────────────────


def derive_responses_function_call_id(
    agent: Any,
    call_id: str,
...
def chat_messages_to_responses_input(
    ...
    from run_agent import _chat_content_to_responses_parts
...
def preflight_codex_input_items(
    agent: Any,
    raw_items: Any,
) -> List[Dict[str, Any]]:
    ...
    for idx, item in enumerate(raw_items):
        ...
        if role in {"user", "assistant"}:
            ...
            if content is None:
                content = ""
            if isinstance(content, list):
                # Multimodal content from ``chat_messages_to_responses_input``
...
def preflight_codex_api_kwargs(
    agent: Any,
    api_kwargs: Any,
    *,
    ...
    normalized_input = preflight_codex_input_items(agent, api_kwargs.get("input"))
...
def extract_responses_message_text(agent: Any, item: Any) -> str:
...
def extract_responses_reasoning_text(agent: Any, item: Any) -> str:
...
def normalize_codex_response(
    ...
    for item in output:
        ...
        if item_type == "message":
            ...
            message_text = extract_responses_message_text(agent, item)
            if message_text:
                content_parts.append(message_text)
        elif item_type == "reasoning":
            reasoning_text = extract_responses_reasoning_text(agent, item)
            if reasoning_text:
            ...
            response_item_id = derive_responses_function_call_id(agent, call_id, response_item_id)
