...
def has_pending(session_key: str) -> bool:
    """Check if a session key has a pending approval request."""
    with _lock:
        return session_key in _pending


def pop_pending(session_key: str) -> Optional[dict]:
    """Remove and return the pending approval request for a session."""
    with _lock:
        return _pending.pop(session_key, None)


def inject_test_approval(session_key: str, approval: dict) -> None:
    """Force inject a pending approval request for testing."""
    with _lock:
        _pending[session_key] = approval
