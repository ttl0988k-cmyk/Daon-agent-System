    ...
    def spawn_local(
        ...
        if use_pty:
            ...
            try:
                ...
                # Windows PTY spawn options - use conhost backend to avoid console window popup
                spawn_kwargs = {
                    "dimensions": (30, 120),
                }
                if _IS_WINDOWS:
                    # conhost backend avoids the visible console window on Windows 10+
                    # Falls back to default winpty backend if conhost is unavailable
                    spawn_kwargs["backend"] = "conhost"
