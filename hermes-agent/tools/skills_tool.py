...
def skills_list(category: str = None, task_id: str = None) -> str:
    ...
    This function uses the centralized skill index (skill_index.py) for fast
    lookups instead of scanning the filesystem each time.
    ...
    """
    try:
        # Try using the new skill index first
        try:
            from tools.skill_index import get_skill_index
            
            index = get_skill_index()
            
            # Check if index needs rebuild
            if not index._index_data or not index._index_path.exists():
                index.rebuild(force=True)
            
            # Get skills from index
            all_skills = index.list_skills(category=category, enabled_only=False)
            ...
            # Fallback to old filesystem scan if skill_index not available
            pass
        ...
        # Find all skills using legacy method
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        # ── Try skill index first for fast lookup ───────────────────────
        skill_dir = None
        skill_md = None
        from_index = False
        ...
        try:
            ...
            index = get_skill_index()
            skill_info = index.get_skill(name)
            
            if skill_info and skill_info.get("path"):
                # Found in index - use the path
                skill_path = index._skills_dir / skill_info["path"]
                if skill_path.exists() and (skill_path / "SKILL.md").exists():
                    skill_dir = skill_path
                    skill_md = skill_path / "SKILL.md"
                    from_index = True
                    logger.debug(f"skill_view: found '{name}' in index at {skill_md}")
        except ImportError:
            pass  # Fallback to filesystem scan
        except Exception as e:
            logger.debug(f"skill_view: index lookup failed, falling back to filesystem: {e}")
        
        # ── Fallback: filesystem scan if not found in index ─────────────
        if not skill_md:
            from agent.skill_utils import get_external_skills_dirs
        ...
        if not skill_md or not skill_md.exists():
            # Try to get suggestions from index if available
            try:
                from tools.skill_index import get_skill_index
                index = get_skill_index()
                available = [s["id"] for s in index.list_skills()[:20]]
            except Exception:
