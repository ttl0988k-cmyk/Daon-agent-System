...
def _build_child_progress_callback(task_index: int, goal: str, parent_agent, task_count: int = 1) -> Optional[callable]:
    ...
    def _callback(event_type: str, tool_name: str = None, preview: str = None, args=None, **kwargs):
        ...
        # ── 다온 에이전트 오피스 실시간 시각화 브릿지 연동 패치 ──────────────────
        try:
            office_bridges_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "daon-agent-office", "bridges")
            if os.path.exists(office_bridges_dir):
                import re
                base_name = str(goal_label or "").split(' (')[0]
                safe_name = re.sub(r'[^a-zA-Z0-9가-힣]', '', base_name)
                agent_id = f"dyn_{safe_name}"

                NEON_COLORS = {
                    "researcher": 0x06b6d4,
                    "writer": 0xf43f5e,
                    "designer": 0x10b981,
                    "layout-builder": 0x3b82f6,
                    "reviewer": 0xa78bfa
                }
                color = NEON_COLORS.get(agent_id, 0x10b981)
                ...
                if event_type == "subagent.start":
                    status = "thinking"
                    log_msg = f"작업 분석 및 기획 개시: {goal_label}"
                    # Dynamic Spawning 패킷 발행
                    spawn_packet = {
                        "action": "spawn",
                        "agent": agent_id,
                        "config": {
                            "id": agent_id,
                            "name": agent_id,
                            "role": goal_label[:30] if goal_label else "자율 에이전트",
                            "color": color
                        },
                        "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
                    }
                    spawn_fpath = os.path.join(office_bridges_dir, f"spawn_{agent_id}.json")
                    with open(spawn_fpath, "w", encoding="utf-8") as f:
                        import json as _json
                        _json.dump(spawn_packet, f, indent=2, ensure_ascii=False)

                elif event_type in ("_thinking", "reasoning.available"):
                    status = "thinking"
                    log_msg = preview or "생각 중..."
                    # Thought Stream 패킷 발행
                    if preview:
                        thought_packet = {
                            "action": "thought",
                            "agent": agent_id,
                        ...
                        thought_fpath = os.path.join(office_bridges_dir, f"thought_{agent_id}.json")
                        with open(thought_fpath, "w", encoding="utf-8") as f:
                            import json as _json
                            _json.dump(thought_packet, f, indent=2, ensure_ascii=False)

                elif event_type == "tool.started":
                    status = "typing"
                    log_msg = f"도구 실행 중: {tool_name}"
                    if preview:
                        log_msg += f" ({preview})"
                    # Tool Call Execution 패킷 발행
                    if tool_name:
                        args_snap = {}
                        ...
                        tool_fpath = os.path.join(office_bridges_dir, f"tool_{agent_id}.json")
                        with open(tool_fpath, "w", encoding="utf-8") as f:
                            import json as _json
                            _json.dump(tool_packet, f, indent=2, ensure_ascii=False)
                ...
                fpath = os.path.join(office_bridges_dir, f"{agent_id}.json")
                with open(fpath, "w", encoding="utf-8") as f:
                    import json as _json
                    _json.dump(packet, f, indent=2, ensure_ascii=False)
        except Exception as _e:
            logger.debug("Office status write failed: %s", _e)
        # ──────────────────────────────────────────────────────────────────
