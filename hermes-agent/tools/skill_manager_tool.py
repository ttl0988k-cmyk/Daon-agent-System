...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        index.rebuild(force=True)  # Rebuild to include new skill
    except Exception as e:
        logger.debug(f"Failed to update skill index after create: {e}")
...
def _edit_skill(name: str, content: str) -> Dict[str, Any]:
    ...
    try:
        from tools.skill_index import get_skill_index
        index = get_skill_index()
        index.rebuild(force=True)  # Rebuild to reflect content changes
    except Exception as e:
        logger.debug(f"Failed to update skill index after edit: {e}")
...
def _patch_skill(
    ...
    # Update skill index if SKILL.md was patched (not supporting files)
    if not file_path:
        try:
            from tools.skill_index import get_skill_index
            index = get_skill_index()
            ...
            logger.debug(f"Failed to update skill index after patch: {e}")
...
def _delete_skill(name: str) -> Dict[str, Any]:
    ...
    try:
        from tools.skill_index import get_skill_index
        index = get_skill_index()
        index.rebuild(force=True)  # Rebuild to remove deleted skill
    except Exception as e:
        logger.debug(f"Failed to update skill index after delete: {e}")
