...
class SessionDB:
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 4:
                ...
                try:
                    cursor.execute(
                        "CREATE INDEX IF NOT EXISTS idx_sessions_title_unique "
                        "ON sessions(title)"
                    )
            ...
            if current_version < 7:
                # v7: add updated_at column to sessions table for WebUI session sync
                try:
                    cursor.execute("ALTER TABLE sessions ADD COLUMN updated_at REAL")
                except sqlite3.OperationalError:
                    pass  # Column already exists
    ...
    def update_system_prompt(self, session_id: str, system_prompt: str, model: str = None) -> None:
        """Store the full assembled system prompt snapshot.

        Optionally updates the model column at the same time so that
        model-change detection in streaming.py stays accurate after
        the new system prompt is built.
        """
        def _do(conn):
            if model is not None:
                conn.execute(
                    "UPDATE sessions SET system_prompt = ?, model = ? WHERE id = ?",
                    (system_prompt, model, session_id),
                )
            else:
                conn.execute(
