...
Add MCP servers to `data/mcp_servers.json` under the `mcp_servers` key:
...
1. Reads `mcp_servers` from `data/mcp_servers.json`
...
No `mcp_servers` key in `data/mcp_servers.json`, or it's empty. Add at least one server.
