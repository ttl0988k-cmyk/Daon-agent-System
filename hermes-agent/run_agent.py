...
class AIAgent:
    ...
    def __init__(
        ...
        elif self.provider in {"qwen-token-plan", "qwen"} or "compatible-mode" in self._base_url_lower:
            self.api_mode = "chat_completions"
        elif self._base_url_lower.rstrip("/").endswith("/anthropic"):
            # Third-party Anthropic-compatible endpoints (e.g. MiniMax)
            # use a URL convention ending in /anthropic.
            self.api_mode = "anthropic_messages"
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        try:
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                # Bypassed strict check for openrouter/free models that misreport
                print(
                    f"WARNING: Auxiliary compression model {aux_model} reported a context "
                    f"window of {aux_context:,} tokens, which is below the "
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,}. Proceeding anyway."
                )
                # Override to minimum to allow compression to run
    ...
    def _looks_like_intermediate_ack(
        ...
        """Detect a planning/ack message (in English or Korean) that should continue instead of ending the turn."""
        # NOTE: Previously this function returned False if any tool message existed in messages.
        # That check was REMOVED because it caused the bug where after tool execution,
        # even a genuine intermediate ACK like "확인해볼게요" would not trigger auto-continue.
        # The remaining logic (action_markers + has_future_ack) is sufficient to avoid
        # false positives while properly detecting intermediate ACKs after tool execution.
        ...
        has_future_ack = bool(
            re.search(
                ...
                r"(확인(해|하|해\s*볼|해\s*보겠습니다|하겠습니다|중|할게요|해볼게요)|"
                r"살펴(볼게요|보겠습니다|보죠)|검토(해볼게요|하겠습니다|해보겠습니다)|"
                r"분석(해볼게요|하겠습니다|해보겠습니다)|조회(해볼게요|하겠습니다|해보겠습니다)|"
                r"조사(해볼게요|하겠습니다|해보겠습니다)|체크(해볼게요|하겠습니다|해보겠습니다)|"
                r"진행(해볼게요|하겠습니다|해보겠습니다)|보겠습니다)",
                assistant_text,
                re.IGNORECASE,
            )
        )
        ...
        return (user_targets_workspace or assistant_targets_workspace or has_future_ack) and assistant_mentions_action

    _looks_like_codex_intermediate_ack = _looks_like_intermediate_ack
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Response & Execution Communication Requirement
        prompt_parts.append(
            "IMPORTANT RESPONSE & TOOL EXECUTION GUIDELINES:\n"
            "1. NEVER end a conversation turn by only saying 'I will check' or '확인해볼게요'. Immediately proceed to execute the required tools or provide the analysis.\n"
            "2. After executing tools (such as terminal commands, file reads/writes, or searches), ALWAYS provide a clear, complete summary of the changes, outputs, and final results to the user. Do not stop without explaining what was done and the final outcome."
        )
    ...
    def _deterministic_call_id(fn_name: str, arguments: str, index: int = 0) -> str:
        ...
        """
        from agent.codex_adapter import _deterministic_call_id as _impl
        return _impl(fn_name, arguments, index)

    @staticmethod
    def _split_responses_tool_id(raw_id: Any) -> tuple[Optional[str], Optional[str]]:
        """Split a stored tool id into (call_id, response_item_id)."""
        from agent.codex_adapter import _split_responses_tool_id as _impl
        return _impl(raw_id)
    ...
    def _derive_responses_function_call_id(
        ...
        from agent.codex_adapter import derive_responses_function_call_id as _impl
        return _impl(self, call_id, response_item_id)

    def _chat_messages_to_responses_input(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Convert internal chat-style messages to Responses input items."""
        from agent.codex_adapter import chat_messages_to_responses_input as _impl
        return _impl(self, messages)

    def _preflight_codex_input_items(self, raw_items: Any) -> List[Dict[str, Any]]:
        """Validate and normalize Responses API input items."""
        from agent.codex_adapter import preflight_codex_input_items as _impl
        return _impl(self, raw_items)
    ...
    def _preflight_codex_api_kwargs(
        ...
        *,
        ...
        """Normalize and validate Responses API keyword arguments."""
        from agent.codex_adapter import preflight_codex_api_kwargs as _impl
        return _impl(self, api_kwargs, allow_stream=allow_stream)

    def _extract_responses_message_text(self, item: Any) -> str:
        """Extract assistant text from a Responses message output item."""
        from agent.codex_adapter import extract_responses_message_text as _impl
        return _impl(self, item)

    def _extract_responses_reasoning_text(self, item: Any) -> str:
        """Extract a compact reasoning text from a Responses reasoning item."""
        from agent.codex_adapter import extract_responses_reasoning_text as _impl
        return _impl(self, item)

    def _normalize_codex_response(self, response: Any) -> tuple[Any, str]:
        """Normalize a Responses API object to an assistant_message-like object."""
        from agent.codex_adapter import normalize_codex_response as _impl
        return _impl(self, response)

    def _thread_identity(self) -> str:
        from agent.openai_client_lifecycle import thread_identity
        return thread_identity(self)

    def _client_log_context(self) -> str:
        from agent.openai_client_lifecycle import client_log_context
        return client_log_context(self)

    def _openai_client_lock(self) -> threading.RLock:
        from agent.openai_client_lifecycle import openai_client_lock
        return openai_client_lock(self)

    @staticmethod
    def _is_openai_client_closed(client: Any) -> bool:
        from agent.openai_client_lifecycle import is_openai_client_closed
        return is_openai_client_closed(None, client)

    @staticmethod
    def _build_keepalive_http_client() -> Any:
        from agent.openai_client_lifecycle import build_keepalive_http_client
        return build_keepalive_http_client(None)

    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        from agent.openai_client_lifecycle import create_openai_client
        return create_openai_client(self, client_kwargs, reason=reason, shared=shared)

    @staticmethod
    def _force_close_tcp_sockets(client: Any) -> int:
        from agent.openai_client_lifecycle import force_close_tcp_sockets
        return force_close_tcp_sockets(None, client)

    def _close_openai_client(self, client: Any, *, reason: str, shared: bool) -> None:
        from agent.openai_client_lifecycle import close_openai_client
        return close_openai_client(self, client, reason=reason, shared=shared)

    def _replace_primary_openai_client(self, *, reason: str) -> bool:
        from agent.openai_client_lifecycle import replace_primary_openai_client
        return replace_primary_openai_client(self, reason=reason)

    def _ensure_primary_openai_client(self, *, reason: str) -> Any:
        from agent.openai_client_lifecycle import ensure_primary_openai_client
        return ensure_primary_openai_client(self, reason=reason)

    def _cleanup_dead_connections(self) -> bool:
        from agent.openai_client_lifecycle import cleanup_dead_connections
        return cleanup_dead_connections(self)

    def _create_request_openai_client(self, *, reason: str) -> Any:
        from agent.openai_client_lifecycle import create_request_openai_client
        return create_request_openai_client(self, reason=reason)

    def _close_request_openai_client(self, client: Any, *, reason: str) -> None:
        from agent.openai_client_lifecycle import close_request_openai_client
        return close_request_openai_client(self, client, reason=reason)

    def _run_codex_stream(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        """Execute one streaming Responses API request and return the final response."""
        from agent.stream_handler import run_codex_stream
        return run_codex_stream(self, api_kwargs, client=client, on_first_delta=on_first_delta)

    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        """Fallback path for stream completion edge cases on Codex-style Responses backends."""
        from agent.stream_handler import run_codex_create_stream_fallback
        return run_codex_create_stream_fallback(self, api_kwargs, client=client)

    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        from agent.fallback import try_refresh_codex_client_credentials
        return try_refresh_codex_client_credentials(self, force=force)

    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        from agent.fallback import try_refresh_nous_client_credentials
        return try_refresh_nous_client_credentials(self, force=force)

    def _try_refresh_anthropic_client_credentials(self) -> bool:
        from agent.fallback import try_refresh_anthropic_client_credentials
        return try_refresh_anthropic_client_credentials(self)

    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        from agent.fallback import apply_client_headers_for_base_url
        return apply_client_headers_for_base_url(self, base_url)

    def _swap_credential(self, entry) -> None:
        from agent.fallback import swap_credential
        return swap_credential(self, entry)
    ...
    def _recover_with_credential_pool(
        ...
        *,
        ...
        from agent.fallback import recover_with_credential_pool
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        provider fallback.
        """
        from agent.stream_handler import interruptible_api_call
    ...
    def _reset_stream_delivery_tracking(self) -> None:
        ...
        from agent.stream_handler import reset_stream_delivery_tracking
        reset_stream_delivery_tracking(self)

    def _record_streamed_assistant_text(self, text: str) -> None:
        """Accumulate visible assistant text emitted through stream callbacks."""
        from agent.stream_handler import record_streamed_assistant_text
        record_streamed_assistant_text(self, text)

    @staticmethod
    def _normalize_interim_visible_text(text: str) -> str:
        from agent.stream_handler import normalize_interim_visible_text
        return normalize_interim_visible_text(text)

    def _interim_content_was_streamed(self, content: str) -> bool:
        from agent.stream_handler import interim_content_was_streamed
        return interim_content_was_streamed(self, content)

    def _emit_interim_assistant_message(self, assistant_msg: Dict[str, Any]) -> None:
        """Surface a real mid-turn assistant commentary message to the UI layer."""
        from agent.stream_handler import emit_interim_assistant_message
        emit_interim_assistant_message(self, assistant_msg)

    def _fire_stream_delta(self, text: str) -> None:
        """Fire all registered stream delta callbacks (display + TTS)."""
        from agent.stream_handler import fire_stream_delta
        fire_stream_delta(self, text)

    def _fire_reasoning_delta(self, text: str) -> None:
        """Fire reasoning callback if registered."""
        from agent.stream_handler import fire_reasoning_delta
        fire_reasoning_delta(self, text)

    def _fire_tool_gen_started(self, tool_name: str) -> None:
        """Notify display layer that the model is generating tool call arguments."""
        from agent.stream_handler import fire_tool_gen_started
        fire_tool_gen_started(self, tool_name)

    def _has_stream_consumers(self) -> bool:
        """Return True if any streaming consumer is registered."""
        from agent.stream_handler import has_stream_consumers
        return has_stream_consumers(self)

    def _interruptible_streaming_api_call(
        self, api_kwargs: dict, *, on_first_delta: callable = None
    ):
        """Streaming variant of _interruptible_api_call for real-time token delivery."""
        from agent.stream_handler import interruptible_streaming_api_call
    ...
    def _try_activate_fallback(self) -> bool:
        ...
        mappings.
        """
        from agent.fallback import try_activate_fallback as _try_activate_fallback
        return _try_activate_fallback(self)
    ...
    def _restore_primary_runtime(self) -> bool:
        ...
        """
        from agent.fallback import restore_primary_runtime as _restore_primary_runtime
        return _restore_primary_runtime(self)
    ...
    def _try_recover_primary_transport(
        ...
        """
        from agent.fallback import try_recover_primary_transport as _try_recover_primary_transport
        return _try_recover_primary_transport(self, api_error, retry_count=retry_count, max_retries=max_retries)
    ...
    def _execute_tool_calls(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        """
        from agent.tool_executor import execute_tool_calls as _execute_tool_calls
        return _execute_tool_calls(self, assistant_message, messages, effective_task_id, api_call_count)
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        """
        from agent.tool_executor import invoke_tool as _invoke_tool
        return _invoke_tool(self, function_name, function_args, effective_task_id, tool_call_id=tool_call_id)
    ...
    def _wrap_verbose(label: str, text: str, indent: str = "     ") -> str:
        ...
        """
        from agent.tool_executor import wrap_verbose as _wrap_verbose
        return _wrap_verbose(None, label, text, indent=indent)
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        """
        from agent.tool_executor import execute_tool_calls_concurrent as _execute_tool_calls_concurrent
        return _execute_tool_calls_concurrent(self, assistant_message, messages, effective_task_id, api_call_count)

    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        """Execute tool calls sequentially (original behavior). Used for single calls or interactive tools."""
        from agent.tool_executor import execute_tool_calls_sequential
        execute_tool_calls_sequential(self, assistant_message, messages, effective_task_id, api_call_count)



    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        """Request a summary when max iterations are reached. Returns the final response text."""
        from agent.tool_executor import handle_max_iterations
    ...
    def run_conversation(
        ...
        if self._cached_system_prompt is None:
            ...
            if stored_prompt:
                ...
                # Store the system prompt snapshot in SQLite (include current model for
                # model-change detection in streaming.py)
                if self._session_db:
                    try:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if (
                            ...
                            messages.append({
                                "role": "user",
                                "content": (
                                    "You just executed tool calls (such as terminal or file operations) but returned an "
                                    "empty response or did not summarize the results. Please process the tool "
                                    "results above and provide a clear, complete summary of the changes and execution results for the user.\n"
                                    "[시스템 안내: 터미널/파일 작업 실행 결과를 바탕으로 변경 사항과 결과를 명확하고 자세하게 요약하여 최종 응답을 작성하세요.]"
                                ),
                            })
