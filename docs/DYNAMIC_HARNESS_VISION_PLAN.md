...
- **D-2 (보류 — 대표님 결정 2026-08-17)**: 깊이 > 1 허용은 당분간 하지 않기로 결정.
  근거: (1) 대부분의 작업 분해는 깊이가 아니라 넓이(서브팀 규모)로 해결 가능,
  (2) 동기 실행 위임은 깊이가 늘수록 토큰/시간 비용과 대기 시간이 곱셈으로 증가,
  (3) 초보 바이브코더 대상 도구에서 3층 이상 조직 구조의 실수요가 거의 없음.
  구조는 이미 깊이 대비 완료 상태 — 가드가 `harness_limits.yaml`의
